...
-----------------------------------------------------------------------------------------------------------------------------------------
Lume = {}
Tunnel.bindInterface("garages", Lume)
...
-- SERVERVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Lume.ServerVehicle(Model, x, y, z, Heading, Plate, Nitrox, Doors, Body, Fuel)
...
-- SPAWN
-----------------------------------------------------------------------------------------------------------------------------------------
function Lume.Spawn(Name, Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
					...
					if Coords then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								...
								local Exist, Network = Lume.ServerVehicle(Name, Coords[1], Coords[2], Coords[3],
								...
								elseif vehicle[1]["mode"] == "normal" then
									...
									if vRP.HasGroup(Passport, "Premium") then
										local Exist, Network = Lume.ServerVehicle(Name, Coords[1], Coords[2], Coords
										...
										if vRP.Request(source, "Garagem", "Retirar o veículo por <b>$" .. parseFormat(VehiclePrice * 0.05) .. "</b> dólares?") then
											if vRP.PaymentFull(Passport, parseInt(VehiclePrice * 0.05)) then
												local Exist, Network = Lume.ServerVehicle(Name, Coords[1], Coords[2],
							...
							TriggerClientEvent("garages:Close", source)
							local Exist, Network = Lume.ServerVehicle(Name, Coords[1], Coords[2], Coords[3], Coords[4],
...
RegisterCommand("car", function(source, Message)
	...
	if Passport then
		if vRP.HasGroup(Passport, "Admin") and Message[1] then
			...
			local Exist, Network, Vehicle = Lume.ServerVehicle(VehicleName, Coords["x"], Coords["y"], Coords["z"],
...
-- DELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function Lume.Delete(Network, Health, Engine, Body, Fuel, Doors, Windows, Tyres, Plate, Brake)
	if Spawn[Plate] then
